...
export default function Agenda() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await apiPut(`/agendamentos/${id}`, { status: newStatus })
      refetch()
    } catch (error) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'atendido': return 'bg-yellow-100 text-yellow-800'
      case 'concluido': return 'bg-gray-100 text-gray-800'
      case 'remarcado': return 'bg-orange-100 text-orange-800'
      case 'cancelado': return 'bg-red-100 text-red-800'
      case 'faltou': return 'bg-red-200 text-red-900'
  ...
  return (
    <div>
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-xl">
        <div className="bg-gradient-to-r from-cyan-600 to-blue-600 p-4 rounded-t-xl">
          <h2 className="font-semibold text-white">Agendamentos - {format(new Date(selectedDate), 'dd/MM/yyyy')}</h2>
        </div>
            ...
            <div key={agendamento.id} className="p-4 flex items-center justify-between hover:bg-gray-50">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className={`px-2 py-1 rounded text-xs border-0 ${getStatusColor(agendamento.status)}`}
                >
